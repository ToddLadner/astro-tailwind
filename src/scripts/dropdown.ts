...
export const initDropdown = (root: Element | null) => {
  ...
  const getItems = () =>
    Array.from(menu.querySelectorAll<HTMLElement>('[role^="menuitem"]')).filter(isFocusable);
...
export const initAllDropdowns = (scope?: ParentNode | Document) => {
  const rootScope =
    scope ?? (typeof document !== 'undefined' ? (document as ParentNode) : undefined);
  rootScope
    ?.querySelectorAll('[data-dropdown-root]')
